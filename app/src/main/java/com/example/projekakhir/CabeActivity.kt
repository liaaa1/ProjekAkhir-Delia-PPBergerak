...
class CabeActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_cabe)

        val detailsayur = findViewById<RecyclerView>(R.id.rvhijau)
        ApiConfig.getService().getDetail2().enqueue(object : Callback<ResponseCabai> {
            override fun onResponse(
                call: Call<ResponseCabai>, response: Response<ResponseCabai>
            ) {
                if (response.isSuccessful) {
                    ...
                    detailsayur.apply {
                        layoutManager = LinearLayoutManager(this@CabeActivity)
            ...
            override fun onFailure(call: Call<ResponseCabai>, t: Throwable) {
